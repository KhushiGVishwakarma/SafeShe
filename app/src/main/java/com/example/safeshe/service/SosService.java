package com.example.safeshe.service;
...
import com.example.safeshe.SafeShe;
import com.example.safeshe.api.NotificationAPI;
import com.example.safeshe.common.Constants;
import com.example.safeshe.config.Prefs;
import com.example.safeshe.model.ContactModel;
import com.example.safeshe.ui.activity.MainActivity;
import com.example.safeshe.util.FirebaseUtil;
import com.example.safeshe.util.NotificationClient;
        ...
        if (isGPSEnabled() || !mLocation.isEmpty()) {
            ...
            LocationServices.getFusedLocationProviderClient(this)
                    .requestLocationUpdates(locationRequest, new LocationCallback() {
                        ...
                        public void onLocationResult(@NonNull LocationResult locationResult) {
                            ...
                            if (numberOfUpdates[0] >= 3) {
                                ...
                                if (!locationResult.getLocations().isEmpty()) {
                                    ...
                                    if (Prefs.getBoolean(Constants.SETTINGS_SEND_EMERGENCY_SMS, true) && !sentSMS) {
                                        sendSMS(contacts); // Existing functionality
                                        sentSMS = true;
                                    }
                                    
                                    if (Prefs.getBoolean(Constants.SETTINGS_SEND_NEARBY_ALERTS, false) && !sentSMS) {
                                        sendNearbyEmergencySMS();
                                        sentSMS = true;
                                    }
                        ...
                        private void sendNearbyEmergencySMS() {
                        }
    ...
    private void sendNearbyEmergencySMS() {
        ...
        new Thread(() -> {
            try {
                ...
                for (int i = 0; i < Math.min(results.length(), 3); i++) { // Send to top 3 locations
                    JSONObject place = results.getJSONObject(i);
                    String name = place.getString("name");
                    String address = place.getString("vicinity");

                    // Extract phone number if available
                    String phoneNumber = place.has("formatted_phone_number") ? place.getString("formatted_phone_number") : "N/A";

                    // Send SMS
                    if (!phoneNumber.equals("N/A")) {
                        smsManager.sendTextMessage(phoneNumber, null, "Emergency! I need help at " + mLocation, null, null);
                        Log.i("SosService", "Emergency SMS sent to: " + name);
                    }
                }
            } catch (Exception e) {
                Log.e("SosService", "Error sending emergency SMS: " + e.getMessage());
            }
        }).start();
    }
}